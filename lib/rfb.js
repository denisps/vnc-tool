...
class RFBClient extends EventEmitter {
  ...
  disconnect() {
    ...
    // clear first-screen promise so that any waiter unblocks
  ...
  async _handshake() {
    ...
    // immediately request a full framebuffer update so that callers can
    // await screen data without having to issue the first request
    // lazily.  The promise is fulfilled in _handleFramebufferUpdate().
    this._firstScreenPromise = new Promise((resolve, reject) => {
      this._firstScreenResolve = resolve;
      this._firstScreenReject = reject;
    });
  ...
  captureScreen() {
    ...
    // if we've already seen at least one update, simply return a copy of the
    // framebuffer (no new request is performed).  this mirrors the previous
    // behaviour, but note that the first update is now requested during the
    // handshake instead of lazily here.
    if (this._fb && this._updateCount > 0) {
      return Promise.resolve({
        width: this.width,
        height: this.height,
        rgba: Buffer.from(this._fb),
      });
    }

    // otherwise wait for the promise created at connect time
    if (this._firstScreenPromise) {
      return this._firstScreenPromise.then(() => ({
        width: this.width,
        height: this.height,
        rgba: Buffer.from(this._fb),
      }));
    ...
    // fallback: shouldn't really happen, but if handshake didn't create the
    // promise fall back to issuing a request now.
    return new Promise((resolve, reject) => {
  ...
  async _handleFramebufferUpdate() {
    ...
    // resolve the promise waiting for the very first screen update
    if (this._firstScreenResolve) {
      this._firstScreenResolve();
      this._firstScreenResolve = null;
      this._firstScreenReject = null;
   ...
   *
   * This is a **floating-point ratio**: each RFB rectangle update contributes
   * `changed_pixels / (width*height)` to the counter.  Clients should treat
   * it as a monotonically increasing float rather than an integer frame index.
   * @type {number}
   */
  get updateCount() {
  ...
  /**
   * Return a copy of the current RGBA framebuffer, waiting for the first
   * screen update if none have arrived yet.  This mirrors the semantics of
   * {@link captureScreen} but only returns the raw buffer portion.
   * @returns {Promise<Buffer|null>}
   */
  async screenshotRaw() {
    // ensure at least one update has completed
