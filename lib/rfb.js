 ...
 *
 * The client maintains an internal update loop: after each framebuffer
 * update it sends an incremental request so that servers continue to
 * send changes.  Listeners can subscribe to the `'update'` event and
 * they will receive one event for every frame the server sends. The
 * first request (non-incremental) is issued automatically when the
 * connection is established, so no explicit call is needed to start
 * receiving updates.
 *
 * The `captureScreen()` helper performs a single non-incremental
 * request and returns a promise that resolves with RGBA data. It
 * works seamlessly alongside the continuous loop: subsequent updates
 * still arrive (and are emitted) after the promise has resolved.
 *
...
class RFBClient extends EventEmitter {
  constructor(options = {}) {
    ...
    // bookkeeping for buffering races: if updates arrive before the
    // framebuffer has been allocated we keep them until buffering starts.
    this._preBufferRects = null;         // rects received before _fb exists

    // ensure pixel format/encodings are sent once, only when we actually
    // start dealing with pixel data.
    this._pixelFormatConfigured = false;
  }
  ...
  async _handshake() {
    ...
    // bytes 4-19: server pixel format (overridden by SetPixelFormat later)
  ...
  requestUpdate(incremental, x, y, w, h) {
    // make sure pixel format and encodings are set before any framebuffer
    // data is sent.  this covers the case where callers call
    // `requestUpdate()` manually without ever buffering.
    if (!this._pixelFormatConfigured) {
      this._sendSetPixelFormat();
      this._sendSetEncodings([0]);
      this._pixelFormatConfigured = true;
    }
  ...
  async startScreenBuffering() {
    ...
    // Allocate framebuffer and reset counter.  We don't know whether the
    // server has already sent an update as part of the handshake, so the
    // counter may get bumped below once we apply pending rects.
    ...
    // If we already received an update before the framebuffer was
    // allocated, apply it now and resolve immediately.  This can occur if
    // someone called `requestUpdate()` manually before buffering began.
    if (this._preBufferRects) {
      let changed = 0;
      for (const rect of this._preBufferRects) changed += this._applyRect(rect);
      if (changed > 0 && this.width * this.height > 0) {
        this._updateCount += changed / (this.width * this.height);
      }
      this._preBufferRects = null;
    ...
    // configure pixel format/encodings before we ask for any data
    if (!this._pixelFormatConfigured) {
      this._sendSetPixelFormat();
      this._sendSetEncodings([0]);
      this._pixelFormatConfigured = true;
    ...
    // Send the initial full-screen request now that buffering has started.
  ...
  async _handleFramebufferUpdate() {
    ...
    if (this._fb) {
      ...
      // If buffering has already started we can fulfil the first-screen
      // promise right away.
      ...
      // No framebuffer yet; stash the rectangles so that the first call to
      // startScreenBuffering can apply them.  We don't resolve the
      // pending promise now because the caller will create/allocate the
      // buffer after this update has already been received.
      this._preBufferRects = rects;
    }

    this.emit('update', { width: this.width, height: this.height, rects });

    // Issue another incremental request so that the server continues to
    // stream changes.  This is the behaviour most VNC clients use: after
    // each framebuffer update we ask for the next one.  The request is
    // sent regardless of whether a capture was pending – by the time we
    // get here `_pendingCapture` has already been cleared.  Having an
    // active capture request doesn't interfere because it resolves on the
    // first update and the subsequent incremental responses simply trigger
    // more `update` events.
    if (this._connected) {
      try {
        this.requestUpdate(true, 0, 0, this.width, this.height);
      } catch (err) {
        // ignore; socket may have closed in the meantime
      }
    }
  }
